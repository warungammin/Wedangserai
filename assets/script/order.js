...
function selectOption(element, type) {
    ...
    if (type === 'variant' && element.getAttribute('data-price')) {
        orderData.variantPrice = parseInt(element.getAttribute('data-price'));
...
function showConfirmation() {
    ...
    orderData.size === 'tangung' ? 
    `Tanggung (<span class="price">Rp 3.000</span>)` : 
    `Jumbo (<span class="price">Rp 5.000</span>)`;
    ...
    // Varian
    const variantText = orderData.variant === 'es' ? 
        'Es' : 
        `Hangat (+<span class="price">Rp ${orderData.variantPrice.toLocaleString('id-ID')}</span>)`;
    document.getElementById('summary-variant').innerHTML = variantText;

    
    // Jumlah
    document.getElementById('summary-quantity').textContent = 
        `${orderData.quantity} cup`;
    
    // Ongkos kirim
    const deliveryElement = document.getElementById('summary-delivery');
    deliveryElement.textContent = deliveryFee === 0 ? 'Gratis' : `Rp ${deliveryFee.toLocaleString('id-ID')}`;

    // Tambahkan class yang sesuai
    const deliveryItem = document.querySelector('#summary-delivery').parentElement;
...
function sendOrder() {
    ...
    const message = `Halo Serai Tea, saya mau pesan:
    
Nama: ${name}
Ukuran Cup: ${orderData.size === 'tangung' ? 'Tanggung' : 'Jumbo'} (Rp ${orderData.sizePrice.toLocaleString('id-ID')})
Rasa: ${orderData.taste === 'manis' ? 'Manis' : 'Tawar'}
Varian: ${orderData.variant === 'es' ? 'Es' : 'Hangat'} ${orderData.variantPrice > 0 ? `(+Rp ${orderData.variantPrice.toLocaleString('id-ID')})` : ''}
Jumlah: ${orderData.quantity} cup
Ongkos Kirim: ${deliveryFee === 0 ? 'Gratis' : `Rp ${deliveryFee.toLocaleString('id-ID')}`}
